...
public class Command_OLD {
    ...
    public Command_OLD(String command) {
        this.command = command;
    }

    public Command_OLD(String command, int id) {
        this.id = id;
        this.command = command;
    }

    public Command_OLD(String command, String description, String syntax, int minimumArgCount, int rank) {
        this.command = command;
    ...
    public boolean checkCommand(Message message) {
        ...
        String prefix = "!";
        try {
            prefix = SherlockBot.guildMap.get(guildID).getPrefix();
        } catch(NullPointerException nullPointerException){
            //System.out.println("NULL POINTER FOUND FOR PREFIX");
        }
